...
def process_single_image(image): # 每一张图片添加 prompt, 以及用 Processor处理成为 llm 能够处理的格式
    """single image"""
    prompt_in = prompt
    cache_item = {
        "prompt": prompt_in,
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor: # PDF 的每一
        batch_inputs = list(tqdm(
            executor.map(process_single_image, images), # 线程池
            total=len(images),
            desc="Pre-processed images"
        ))

    outputs_list = llm.generate( # 经过
        batch_inputs, 
